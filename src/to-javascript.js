...
// Replace import paths to output files generated by psc with paths
...
function makeJS(psModule, psModuleMap, js) {
  const fromRE = /from\s+['"]\.\.\/([\w\.]+)(?:\/index\.js)?['"]/g;
  const foreignRE = /from\s+['"]\.\/([\w\.]+)(?:\/foreign\.js)?['"]/g;
  ...
  const result = js
    .replace(fromRE, (m, p1) => {
      ...
      if (!moduleValue) {
        debug('module %s was not found in the map, replacing import with null', p1);
